...
const RESOURCE_PREVIEW_STORAGE_KEY = 'pb.resource.preview.enabled';
// 一次性迁移：旧键 poetize.resource.preview.enabled → 新键 pb.resource.preview.enabled
try {
  const oldVal = localStorage.getItem('poetize.resource.preview.enabled');
  if (oldVal !== null && localStorage.getItem(RESOURCE_PREVIEW_STORAGE_KEY) === null) {
    localStorage.setItem(RESOURCE_PREVIEW_STORAGE_KEY, oldVal);
    localStorage.removeItem('poetize.resource.preview.enabled');
  }
} catch (e) { /* 忽略迁移失败 */ }
