...
CREATE TABLE `poetize`.`web_info` (
  ...
  `logo_image` varchar(512) DEFAULT NULL COMMENT '网站Logo图片URL（前台导航栏优先显示Logo，为空时显示网站名称）',
...
CREATE TABLE `poetize`.`resource_migration_item` (
  ...
  `source_location_version` int DEFAULT NULL COMMENT '任务创建时活动副本版本',
  ...
  `source_expected_hash` char(64) DEFAULT NULL COMMENT '任务创建时资源基准SHA-256',
  `source_hash_source` varchar(32) DEFAULT NULL COMMENT '任务创建时基准哈希来源',
  `source_hash` char(64) DEFAULT NULL COMMENT '源快照SHA-256',
  `source_size` bigint DEFAULT NULL COMMENT '任务创建时源副本字节数',
  `source_mime_type` varchar(256) DEFAULT NULL COMMENT '任务创建时源副本MIME类型',
...
CREATE TABLE `poetize`.`history_info` (
  ...
  `referer` varchar(512) DEFAULT NULL COMMENT 'HTTP Referer 来源页地址(用于来源站点统计)',
...
--    - summary_config.summaryMode决定摘要生成方式（disabled/global/dedicated/textrank）
--
-- 安全注意事项：
