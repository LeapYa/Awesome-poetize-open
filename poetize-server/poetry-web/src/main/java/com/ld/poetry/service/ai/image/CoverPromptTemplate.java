...
public final class CoverPromptTemplate {
    ...
    /** 标准模式输出指令：约50至80词，适配 qwen-image-2.0-pro 等输入受限的模型（≈77 token） */
    private static final String OUTPUT_RULE_STANDARD =
            "请将以上要素融合成一段连贯的英文描述，约50至80词，信息密度高，避免冗余修饰。";

    /** 详细模式输出指令：约200至400词，适配 qwen-image-3 等支持大输入的模型 */
    private static final String OUTPUT_RULE_DETAILED =
            "请将以上要素融合成一段连贯的英文描述，约200至400词，充分展开每个维度的细节（材质纹理、光影层次、构图元素、色彩搭配等），信息密度高，避免空洞修饰。";

    /** 默认标签示例（物品类/毛毡/赛博朋克/水彩/水墨/像素/3D/极简/拼贴） */
    private static final String LABELS_DEFAULT = "\u201c【基础设定】\u201d、\u201c【核心主体】\u201d";

    /** 人物类标签示例 */
    private static final String LABELS_PORTRAIT = "\u201c【全局画质】\u201d、\u201c【核心人物】\u201d";

    /**
     * 构造输出指令后缀。
     *
     * @param detailed      是否使用详细模式（约200-400词 vs 约50-80词）
     * @param labelExamples 第2条规则中需要禁止输出的前缀标签示例
     */
    private static String outputInstruction(boolean detailed, String labelExamples) {
        String rule1 = detailed ? OUTPUT_RULE_DETAILED : OUTPUT_RULE_STANDARD;
        return "\n\n输出要求：\n" +
                "1. " + rule1 + "\n" +
                "2. 直接输出最终的生图提示词！绝对不要输出" + labelExamples + "等前缀标签，也不要有任何解释说明！";
    }

    // ==================== 获取模板专用 refine_prompt ====================

    /**
     * 根据模板类型返回对应的 LLM 系统提示词（含输出指令）。
     *
     * <p>每个模板的 refine_prompt 由"公式 body"+"输出指令"两部分组成：
     * <ul>
     *   <li>公式 body：定义风格/材质/镜头/光影等维度的提炼规则</li>
     *   <li>输出指令：按 {@code prompt_detail} 配置控制输出词数（standard≈50-80词 / detailed≈200-400词）</li>
     * </ul>
     *
     * <p>模板类型：
     * <ul>
    ...
    public static String getRefinePrompt(ImageConfigDto config) {
        boolean detailed = config.useDetailedPrompt();

        if (config.useFeltTemplate()) {
            return FELT_REFINE_PROMPT + outputInstruction(detailed, LABELS_DEFAULT);
        }
        if (config.useCyberpunkTemplate()) {
            return CYBERPUNK_REFINE_PROMPT + outputInstruction(detailed, LABELS_DEFAULT);
        }
        if (config.useWatercolorTemplate()) {
            return WATERCOLOR_REFINE_PROMPT + outputInstruction(detailed, LABELS_DEFAULT);
        }
        if (config.useInkTemplate()) {
            return INK_REFINE_PROMPT + outputInstruction(detailed, LABELS_DEFAULT);
        }
        if (config.usePixelTemplate()) {
            return PIXEL_REFINE_PROMPT + outputInstruction(detailed, LABELS_DEFAULT);
        }
        if (config.use3dTemplate()) {
            return RENDER3D_REFINE_PROMPT + outputInstruction(detailed, LABELS_DEFAULT);
        }
        if (config.useMinimalTemplate()) {
            return MINIMAL_REFINE_PROMPT + outputInstruction(detailed, LABELS_DEFAULT);
        }
        if (config.useCollageTemplate()) {
            return COLLAGE_REFINE_PROMPT + outputInstruction(detailed, LABELS_DEFAULT);
        }
        if (config.usePortraitTemplate()) {
            return PORTRAIT_REFINE_PROMPT + outputInstruction(detailed, LABELS_PORTRAIT);
        }
        if (config.useCustomTemplate()) {
            String custom = config.getCustomRefinePrompt();
            // 自定义提示词由用户全权控制词数，不追加输出指令；为空时降级为 object 公式
            return (custom != null && !custom.isBlank()) ? custom
                    : OBJECT_REFINE_PROMPT + outputInstruction(detailed, LABELS_DEFAULT);
        }
        return OBJECT_REFINE_PROMPT + outputInstruction(detailed, LABELS_DEFAULT);
    }
