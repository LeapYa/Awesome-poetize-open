...
- **当前初始建库脚本中的表数量**：38 张（以 `poetize-server/sql/poetry.sql` 和 `poetry_old.sql` 为准）
...
|-----|------|------|
| **用户系统** | user | 用户信息 |
| **内容管理** | article, sort, label, article_translation, article_draft, article_draft_collaborator, article_payment | 文章、分类、标签、翻译、草稿、付费 |
| **互动功能** | comment, wei_yan, tree_hole | 评论、微言、树洞 |
| **即时通讯** | im_chat_* | 聊天室相关（6张表） |
| **系统配置** | sys_config, web_info, sys_plugin, sys_plugin_active, sys_ai_config, sys_captcha_config | 系统参数、插件、AI、验证码配置 |
...
| **AI知识库** | ai_knowledge_document | AI知识文档与分块索引 |
| **其他** | family, history_info, db_migrations, plugin_migrations | 家庭页、访问记录、迁移版本、插件迁移版本 |
...
| article_slug | varchar(160) | | NULL | 文章URL别名，留空时使用数字ID |
...
| create_time | datetime | | CURRENT_TIMESTAMP | 创建时间 |
| publish_time | datetime | | NULL | 首次公开发布时间（RSS pubDate 口径，再次隐藏/公开不刷新） |
| update_time | datetime | | CURRENT_TIMESTAMP | 更新时间 |
| update_by | varchar(32) | | NULL | 最终修改人 |
...
- `idx_article_slug (article_slug)` - 保证URL别名唯一并按别名查询文章
- `idx_recommend_status (recommend_status)` - 推荐文章查询
- `idx_view_status (view_status)` - 可见状态过滤
- `idx_article_publish_time (publish_time)` - RSS 按发布时间排序
...
用于承载新建文章草稿与已发布文章的修订草稿。

| 字段 | 类型 | 必填 | 默认值 | 说明 |
|-----|------|-----|-------|------|
| id | varchar(64) | ✓ | | 草稿ID |
| owner_user_id | int | ✓ | | 草稿归属用户ID |
| draft_type | varchar(32) | ✓ | 'CREATE' | 草稿类型 [CREATE, REVISION] |
| article_id | int | | NULL | 关联原文章ID，仅修订草稿使用 |
| status | varchar(32) | ✓ | 'ACTIVE' | 草稿状态 [ACTIVE, PUBLISHING] |
| title_cache | varchar(500) | | NULL | 标题缓存 |
| crdt_snapshot_base64 | mediumtext | | NULL | CRDT 快照 |
| last_editor_id | int | | NULL | 最后编辑人 |
| create_time | datetime | | CURRENT_TIMESTAMP | 创建时间 |
| update_time | datetime | | CURRENT_TIMESTAMP | 更新时间 |
...
**索引**：
- `PRIMARY KEY (id)`
- `UNIQUE KEY uk_article_draft_revision (draft_type, article_id)` - 每篇正式文章最多一个修订草稿
- `idx_owner_status (owner_user_id, status)` - 按归属人和状态查询
- `idx_article_draft_article_id (article_id)` - 按原文章查修订草稿
- `idx_last_editor (last_editor_id)` - 按最后编辑人查询

#### article_draft_collaborator - 草稿协作者表
...
|-----|------|-----|-------|------|
| id | int | ✓ | AUTO | 主键ID |
| draft_id | varchar(64) | ✓ | | 草稿ID |
| user_id | int | ✓ | | 协作者用户ID |
| create_time | datetime | | CURRENT_TIMESTAMP | 创建时间 |
| deleted | tinyint(1) | ✓ | 0 | 逻辑删除 |

**索引**：
- `UNIQUE KEY uk_draft_user (draft_id, user_id)` - 草稿与协作者唯一
- `idx_user_id (user_id)` - 按协作者查询
...
#### ai_knowledge_document - AI知识库文档表

用于存储 AI 聊天知识库文档、分块、向量同步状态等元数据。

| 字段 | 类型 | 必填 | 默认值 | 说明 |
|-----|------|-----|-------|------|
| id | bigint | ✓ | AUTO | 主键ID |
| index_name | varchar(128) | ✓ | 'poetize_ai_chat' | 知识库索引名 |
| document_id | varchar(191) | ✓ | | 文档唯一标识 |
| source_type | varchar(64) | ✓ | | 来源类型 |
| source_id | varchar(128) | ✓ | | 来源业务ID |
| title | varchar(255) | | NULL | 文档标题 |
| content | longtext | ✓ | | 原始内容 |
| visibility_scope | varchar(64) | ✓ | | 可见范围 |
| metadata_json | longtext | | NULL | 扩展元数据 |
| chunk_index | int | ✓ | 0 | 当前分块序号 |
| chunk_count | int | ✓ | 1 | 总分块数 |
| content_hash | varchar(64) | | NULL | 内容哈希 |
| embedding_text | longtext | ✓ | | 向量化文本 |
| sync_status | varchar(32) | ✓ | 'SYNCED' | 同步状态 |
...
- `idx_ai_knowledge_source (index_name, source_type, source_id)`
- `idx_ai_knowledge_scope (index_name, visibility_scope)`
- `idx_ai_knowledge_sync (index_name, last_sync_time)`
...
| resource_hash | varchar(64) | | NULL | 资源内容哈希（SHA-256） |
| status | tinyint(1) | ✓ | 1 | 是否启用 |
...
- `idx_store_hash (store_type, resource_hash)`
...
**支持的平台**：GitHub、Google、Twitter、Yandex、Gitee、QQ、Baidu、爱发电、微博、LinuxDo、Microsoft、GitLab、语雀、华为、小米、Apple、Steam（OpenID 2.0），另支持自定义 OAuth2/OIDC 平台（custom，可对接 Keycloak、Casdoor、Logto 等自建 SSO，通过 authorize_url/token_url/user_info_url 与 uid_field 等字段映射列配置）
...
#### plugin_migrations - 插件 SQL 迁移记录表

用于记录插件安装或升级时执行过的 SQL 文件版本。

| 字段 | 类型 | 必填 | 默认值 | 说明 |
|-----|------|-----|-------|------|
| id | int | ✓ | AUTO | 主键ID |
| plugin_key | varchar(50) | ✓ | | 插件 key |
| version | varchar(20) | ✓ | | 插件版本 |
| sql_file | varchar(128) | | NULL | 已执行 SQL 文件名 |
| executed_at | datetime | | CURRENT_TIMESTAMP | 执行时间 |
| success | tinyint(1) | | 1 | 是否成功 |

**索引**：`UNIQUE KEY uk_plugin_version (plugin_key, version, sql_file)`
...
- 新环境先导入 `poetize-server/sql/poetry.sql` 或 `poetry_old.sql`
- 这两个脚本就是完整建表脚本，默认直接建出当前最新结构
- 安装脚本会创建 `db_migrations`，并把已有增量脚本标记为已执行，避免新装后重复跑迁移

### 2. 增量升级

- 后续升级主要依赖 `poetize-server/sql/` 下按时间戳命名的增量脚本
- 执行入口是 `poetize -update`
- 脚本会按文件名排序，排除 `poetry.sql` 和 `poetry_old.sql`
- 如果 `db_migrations` 已存在，则只执行其中未记录成功的脚本
- 每个脚本执行成功后，会写入 `db_migrations(version, execution_time_ms, success)`
- 执行失败也会记录失败状态，便于排查和重试

### 3. 插件升级

- 插件自己的安装/升级 SQL 不走 `db_migrations`
- 插件升级使用 `plugin_migrations` 记录 `plugin_key + version + sql_file`
- 这样主系统迁移和插件迁移彼此隔离，不会互相污染版本记录

### 4. 当前约束

- 已在线上执行过的迁移脚本原则上不再改历史语义
- 如果需要修正生产逻辑，优先补一条新的增量迁移
- 迁移脚本尽量保持幂等，兼容老环境手工执行和重复执行
